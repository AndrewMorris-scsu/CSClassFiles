 ...
 * 
 * @param memberID
 * @return matching member if found, else null
 */
  public Member searchForMember(String memberID) {
	  Member foundMember = null;
	  Iterator allMembers = getMembers();
	  boolean notFound = true;
	  
	  while (allMembers.hasNext()) {
		  Member temp = (Member)allMembers.next();
  ...
  public Book searchForBook(String BookID) {
	  Book foundBook = null;
	  Iterator allBooks = getBooks();
	  boolean notFound = true;
	  
	  while (allBooks.hasNext()) {
		  Book temp = (Book)allBooks.next();
  ...
  public boolean issueBook(String bookID, String memberID) {
	  ...
	  book = searchForBook(bookID);
	  boolean ableToTake;

    if (book.getCheckedOut() == (false)) {
	    member.issue(book);
	    book.issue(member);
      book.checkoutBook(member);
      ableToTake = true;
	  } else {  
      ableToTake = false;
    }
